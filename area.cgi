...
@gto16=(''
	...
	, '15:20-19:00'
	, '18:20-22:00'
	, '06:20-10:00, 13:50-17:30'
	, '09:20-13:00, 16:50-20:30'
);
@gto17=(''
	, '09:20-13:00, 16:50-20:30'
	, '12:20-16:00'
	...
	if ($getgroup) {
		foreach $_getcity(split(/\t/,$getcity)) {
			if ($areaorg=~ m/$_getcity/ and $num eq $getgroup) {
				$buf.="<tr bgcolor=$bgcolor><td><b>$area1 $area2 $area3</b></td><td>$g18[$num]</td><td>$g19[$num]</td><td>$g20[$num]</td><td>第$numグループ</td></tr>\n";
				++$count;
			}
		}
	} else {
		foreach $_getcity(split(/\t/,$getcity)) {
			if ($areaorg=~ m/$_getcity/) {
				$buf.="<tr bgcolor=$bgcolor><td><b>$area1 $area2 $area3</b></td></td><td>$g18[$num]</td><td>$g19[$num]</td><td>$g20[$num]</td><td>第$numグループ</td></tr>\n";
				++$count;
			}
		}
	}
...
<table border=1><tr bgcolor=#C0C0C0><th>地域</th><th>18日停電時間</th><th>19日停電時間</th><th>20日停電時間</th><th>グループ</th></tr>
$buf
</table><a href=./>戻る</a>
FIN
